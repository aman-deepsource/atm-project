...
def gate_x(t_password):
    ...
    for _ in range(3):  # Limit the try to enter the password
        ...
        if entered_password == "Exit":  # Return the Exit flag
            return "-1"
        if (
            entered_password == t_password
        ):  # Compere if the Entered password = the True password
...
def change_password(ls):
    ...
    if flag == "0":
        ...
        if len(process_list) == 0:  # if there are no processes in the file
            ...
            last_id = (
                int(process_list[len(process_list) - 1][0]) + 1
            )  # get last id and increment it

        id_file.write(
            "{0}\tchange_password\t\t{1}\t{2}\t{3}\n".format(
                str(last_id), str(time.ctime()), old_password, str(new_password)
            )
        )
